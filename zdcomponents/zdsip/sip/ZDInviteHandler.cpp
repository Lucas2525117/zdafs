...
CZDInviteHandler::CZDInviteHandler()
{
}
...
bool CZDInviteHandler::OnReceive(pjsip_rx_data* rdata)
{
	if (PJSIP_INVITE_METHOD == rdata->msg_info.cseq->method.id)
	{
		std::string bodyData = CZDSipContext::GetInstance().GetMessageBody(rdata);
		if (bodyData.empty())
			return false;
		...
		pj_ansi_snprintf(sendSdp, 500,
			...
			"a=sendonly\n"
			"y=%s\n"
			, CZDSipContext::GetInstance().GetSipCode().c_str()
			, CZDSipContext::GetInstance().GetSipIP().c_str()
			, CZDSipContext::GetInstance().GetSipIP().c_str()
