...
bool CZDSipContext::Init(const std::string& concat, int logLevel)
{
...
bool CZDSipContext::RegisterCallback(pjsip_inv_callback* callback)
{
	return pjsip_inv_usage_init(m_endPoint, callback) == PJ_SUCCESS ? true : false;
}

bool CZDSipContext::InitModule()
{
	return pjsip_100rel_init_module(m_endPoint) == PJ_SUCCESS ? true : false;
}

bool CZDSipContext::RegisterModule(pjsip_module* module)
{
...
bool CZDSipContext::UnRegisterModule(pjsip_module* module)
{
	if (!m_endPoint || !module)
		return false;
...
bool CZDSipContext::CreateWorkThread(pj_thread_proc* proc, pj_thread_t* workthread, void* arg, const std::string& threadName)
{
	return pj_thread_create(m_pool, threadName.c_str(), proc, arg, 0, 0, &workthread) == PJ_SUCCESS ? true : false;
}

bool CZDSipContext::DestroyThread(pj_thread_t* thread)
{
	return pj_thread_destroy(thread) == PJ_SUCCESS ? true : false;
}

void CZDSipContext::PJThreadSleep(int msec)
{
	std::this_thread::sleep_for(std::chrono::seconds(1));
	//pj_thread_sleep(msec);
}

void CZDSipContext::HandleEvent(pj_time_val* delay)
{
	pjsip_endpt_handle_events(m_endPoint, delay);
}

bool CZDSipContext::Invite(pjsip_dialog* dlg, GB28181MediaContext mediaContext, std::string sdp)
{
...
bool CZDSipContext::Subscribe(pjsip_dialog* dlg, pjsip_evsub_user* pres_user, const std::string& eventName, const std::string& xmlText, const GBSubscribeContext& subContext)
{
...
void CZDSipContext::Response(pjsip_rx_data* rdata, int st_code, int headType, const std::string& text, int subType)
{
	pjsip_tx_data* tdata;
...
pjsip_response_addr CZDSipContext::GetResponseAddr(pjsip_rx_data* rdata)
{
	pjsip_response_addr addr;
...
void CZDSipContext::QueryDeviceInfo(CZDGBDevice* device, const std::string& gbid, const std::string& scheme)
{
...
void CZDSipContext::QueryRecordInfo(CZDGBDevice* device, const std::string& gbid, const std::string& startTime, const std::string& endTime, const std::string& scheme)
{
...
int CZDSipContext::VoiceBroadcast(CZDGBDevice* device, const std::string& sourceID, const std::string& targetID)
{
...
int CZDSipContext::SendSipMessage(CZDGBDevice* device, const std::string& sipMsg)
{
	pjsip_tx_data* tdata = nullptr;
	const pjsip_method method = { PJSIP_OTHER_METHOD,{ (char*)"MESSAGE", 7 } };
...
int CZDSipContext::PTZControl(CZDGBDevice* device, const std::string& gbid, PTZControlType ptzType, int paramValue)
{
	std::string result = ParsePTZCmd(device, gbid, ptzType, paramValue);
	return SendSipMessage(device, result);
}

std::string CZDSipContext::GetMessageBody(pjsip_rx_data* rdata)
{
...
pj_str_t CZDSipContext::StrToPjstr(const std::string& input)
{
	pj_str_t pjStr;
...
std::string CZDSipContext::GetSipIP() const
{
...
std::string CZDSipContext::GetSipCode() const
{
	RecursiveGuard mtx(rmutex_);
	size_t start = m_concat.find_first_of(":");
...
std::string CZDSipContext::GetLocalDomain()
{
	size_t start = m_concat.find_first_of(":");
	int length = 10;
...
std::string CZDSipContext::ParsePTZCmd(CZDGBDevice* device, const std::string& gbid, PTZControlType ptzType, int paramValue)
{
